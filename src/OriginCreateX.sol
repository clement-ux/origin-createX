...
contract OriginCreateX is Auth {
    constructor(address _governor) Auth(_governor, Authority(address(0))) {}
     ...
     * @return newContract The 20-byte address where the contract was deployed.
     */
    function deployCreate2(bytes memory initCode) public payable requiresAuth returns (address newContract) {
    ...
    ) public payable requiresAuth returns (address newContract) {
        bytes32 guardedSalt = _guard({salt: salt});
        assembly ("memory-safe") {
     ...
     * @return newContract The 20-byte address where the contract was deployed.
     */
    function deployCreate3(bytes memory initCode) public payable requiresAuth returns (address newContract) {
        // Note that the safeguarding function `_guard` is called as part of the overloaded function
        // `deployCreate3`.
