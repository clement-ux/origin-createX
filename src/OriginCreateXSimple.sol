...
contract Authorizer {
    ...
    event Deauthorized(address account);
    ...
    function transferOwnership(address _newOwner) public onlyGovernor {
        governor = _newOwner;
        emit TransferOwnership(_newOwner);
    }

    function authorize(address _account) public onlyGovernor {
        isAuthorized[_account] = true;
        emit Authorized(_account);
    }

    function deauthorize(address _account) public onlyGovernor {
        isAuthorized[_account] = false;
        emit Deauthorized(_account);
    }
}

contract OriginCreateX is Authorizer {
    constructor(address _governor) Authorizer(_governor) {}
    ...
    /**
     * @dev Struct for the `payable requiresAuth` amounts in a deploy-and-initialise call.
     */
    struct Values {
     ...
     * `initCode` must have a `payable requiresAuth` constructor.
     * @param initCode The creation bytecode.
     * @return newContract The 20-byte address where the contract was deployed.
     */
    function deployCreate(bytes memory initCode) public payable onlyAuthorized returns (address newContract) {
     ...
     * creation bytecode `initCode`, the initialisation code `data`, the struct for the `payable requiresAuth`
     ...
     * is non-zero, `initCode` must have a `payable requiresAuth` constructor.
     * @param initCode The creation bytecode.
     * @param data The initialisation code that is passed to the deployed contract.
     * @param values The specific `payable requiresAuth` amounts for the deployment and initialisation call.
     ...
     * `initCode` must have a `payable requiresAuth` constructor, and any excess ether is returned to `msg.sender`.
     * @param initCode The creation bytecode.
     * @param data The initialisation code that is passed to the deployed contract.
     * @param values The specific `payable requiresAuth` amounts for the deployment and initialisation call.
     * @return newContract The 20-byte address where the contract was deployed.
     ...
     * function called via `data` must be `payable requiresAuth`.
     ...
     * must have a `payable requiresAuth` constructor.
     * @param salt The 32-byte random value used to create the contract address.
     * @param initCode The creation bytecode.
     * @return newContract The 20-byte address where the contract was deployed.
     */
     ...
     * Note that if `msg.value` is non-zero, `initCode` must have a `payable requiresAuth` constructor.
     * @param initCode The creation bytecode.
     * @return newContract The 20-byte address where the contract was deployed.
     */
    function deployCreate2(bytes memory initCode) public payable onlyAuthorized returns (address newContract) {
     ...
     * for the `payable requiresAuth` amounts `values`, the refund address `refundAddress`, and `msg.value` as inputs.
     * In order to save deployment costs, we do not sanity check the `initCode` length. Note that if
     * `values.constructorAmount` is non-zero, `initCode` must have a `payable requiresAuth` constructor.
     * @param salt The 32-byte random value used to create the contract address.
     * @param initCode The creation bytecode.
     * @param data The initialisation code that is passed to the deployed contract.
     * @param values The specific `payable requiresAuth` amounts for the deployment and initialisation call.
    ...
    ) public payable onlyAuthorized returns (address newContract) {
        bytes32 guardedSalt = _guard({salt: salt});
        assembly ("memory-safe") {
     ...
     * the `payable requiresAuth` amounts `values`, and `msg.value` as inputs. In order to save deployment costs,
    ...
    /**
     * @dev Deploys and initialises a new contract via calling the `CREATE2` opcode and using the
     * creation bytecode `initCode`, the initialisation code `data`, the struct for the `payable requiresAuth` amounts
     * `values`, and `msg.value` as inputs. The salt value is calculated pseudo-randomly using a
     * diverse selection of block and transaction properties. This approach does not guarantee true
     * randomness! In order to save deployment costs, we do not sanity check the `initCode` length.
     * Note that if `values.constructorAmount` is non-zero, `initCode` must have a `payable requiresAuth` constructor,
     ...
     * `msg.value` is non-zero, the initialiser function called via `data` must be `payable requiresAuth`.
     ...
     * Note that if `msg.value` is non-zero, `initCode` must have a `payable requiresAuth` constructor. This
     ...
     * non-zero, `initCode` must have a `payable requiresAuth` constructor. This implementation is based on Solmate:
     * https://web.archive.org/web/20230921113832/https://raw.githubusercontent.com/transmissions11/solmate/e8f96f25d48fe702117ce76c79228ca4f20206cb/src/utils/CREATE3.sol.
     * @param initCode The creation bytecode.
     * @return newContract The 20-byte address where the contract was deployed.
     */
    function deployCreate3(bytes memory initCode) public payable onlyAuthorized returns (address newContract) {
        // Note that the safeguarding function `_guard` is called as part of the overloaded function
        // `deployCreate3`.
     ...
     * initialisation code `data`, the struct for the `payable requiresAuth` amounts `values`, the refund address
     ...
     * have a `payable requiresAuth` constructor. This implementation is based on Solmate:
     * https://web.archive.org/web/20230921113832/https://raw.githubusercontent.com/transmissions11/solmate/e8f96f25d48fe702117ce76c79228ca4f20206cb/src/utils/CREATE3.sol.
     * @param salt The 32-byte random value used to create the proxy contract address.
     * @param initCode The creation bytecode.
     * @param data The initialisation code that is passed to the deployed contract.
     ...
     * @dev Deploys and initialises a new contract via employing the `CREATE3` pattern (i.e. without
     * an initcode factor) and using the salt value `salt`, the creation bytecode `initCode`, the
     * initialisation code `data`, the struct for the `payable requiresAuth` amounts `values`, and `msg.value` as
     * inputs. In order to save deployment costs, we do not sanity check the `initCode` length. Note
     * that if `values.constructorAmount` is non-zero, `initCode` must have a `payable requiresAuth` constructor,
     ...
     * the struct for the `payable requiresAuth` amounts `values`, the refund address `refundAddress`, and `msg.value`
     ...
     * `initCode` must have a `payable requiresAuth` constructor. This implementation is based on Solmate:
     * https://web.archive.org/web/20230921113832/https://raw.githubusercontent.com/transmissions11/solmate/e8f96f25d48fe702117ce76c79228ca4f20206cb/src/utils/CREATE3.sol.
     * @param initCode The creation bytecode.
     * @param data The initialisation code that is passed to the deployed contract.
     * @param values The specific `payable requiresAuth` amounts for the deployment and initialisation call.
     ...
     * @dev Deploys and initialises a new contract via employing the `CREATE3` pattern (i.e. without
     * an initcode factor) and using the creation bytecode `initCode`, the initialisation code `data`,
     * the struct for the `payable requiresAuth` amounts `values`, `msg.value` as inputs. The salt value is calculated
     ...
     * length. Note that if `values.constructorAmount` is non-zero, `initCode` must have a `payable requiresAuth` constructor,
     * and any excess ether is returned to `msg.sender`. This implementation is based on Solmate:
     * https://web.archive.org/web/20230921113832/https://raw.githubusercontent.com/transmissions11/solmate/e8f96f25d48fe702117ce76c79228ca4f20206cb/src/utils/CREATE3.sol.
     * @param initCode The creation bytecode.
     * @param data The initialisation code that is passed to the deployed contract.
